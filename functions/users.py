...
def all_users(search, status, roll, page, limit, db):
    ...
    # users=db.query(Users).options(joinedload(Users.kpi), joinedload(Users.order.and_(Users.status==True))).filter(search_filter, status_filter, roll_filter).order_by(Users.name.asc())
    users = db.query(Users).options(joinedload(Users.savdolar), joinedload(Users.salary)).filter(search_filter,
                                                                                                 status_filter,
                                                                                                 roll_filter).order_by(
        Users.id.desc())
